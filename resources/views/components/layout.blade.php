...
<body>
  <div class="wrapper-parent">
    ...
    <div id="menu" class="menu-wrap hide">
      <!-- Logo -->
      <div>
        <a class="site-name" href="{{route('dashboard')}}">
          <div class="logo">
    ...
    document.addEventListener("DOMContentLoaded", function () {
        document.querySelectorAll(".status-select").forEach(select => {
            select.addEventListener("change", function () {
                let orderId = this.dataset.orderId; // Assurez-vous que dataset.orderId existe bien
                let newStatus = this.value;
                let csrfToken = document.querySelector('meta[name="csrf-token"]').getAttribute('content');

                fetch(`/order/${orderId}/update-status`, { // Assurez-vous que cette URL correspond à ta route
                    method: "PATCH",
                    headers: {
